...
import { motion, Variants } from "framer-motion"; // 👈 استيراد مكتبة التحريك

export default function Hero() {
  // دالة لتسهيل تأثيرات الدخول المتتابعة للنصوص
  // 2. قم بتعيين النوع ": Variants" للكائن كالتالي 👇
  ...
  return (
    <section
      id="home"
      className="relative min-h-screen flex items-center justify-center overflow-hidden px-6 lg:px-16"
    >
      {/* الوهج الخلفي الملون */}
      ...
      <div className="max-w-7xl w-full mx-auto my-32 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10 text-left">
        ...
        <div className="space-y-6">
          ...
          <motion.div
            ...
            initial="hidden"
            animate="visible"
            custom={0} // العنصر الأول
            className="inline-block px-3 py-1 rounded-full bg-gray-800/40 border border-gray-700/50 backdrop-blur-sm text-sm font-mono text-[#10B981]"
          >
        ...
        {/* العمود اليمين: مجسم الفيزياء والكود مع أنيميشن دخول جانبي وخلفية تفاعلية */}
        <motion.div
          initial={{ opacity: 0, x: 40, scale: 0.95 }} // يدخل من اليمين مع تكبير خفيف
          animate={{ opacity: 1, x: 0, scale: 1 }}
          transition={{ duration: 0.8, delay: 0.6, ease: "easeOut" }} // يظهر بعد اكتمال ظهور النصوص
